    ...
    public class ThirdPersonShooterMotionController : ThirdPersonShooterPlayerScript
    {
        ...
        private void Update()
        {
            ...
            if (playerData.State == ThirdPersonShooterPlayerData.PlayerState.NormalMode)
            {
                ...
                animator.SetFloat("MoveY", inputDampener.CurrentValue.magnitude);
                animator.SetFloat("MoveX", 0);
            }
            else
            {
                animator.SetFloat("MoveY", inputDampener.CurrentValue.y);
                animator.SetFloat("MoveX", inputDampener.CurrentValue.x);
            }
        }
    }
}
